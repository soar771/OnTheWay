...
public class TicketController {
    ...
    @ResponseBody
    public ResponseVo queryStations(PoiSearchRequestVo requestVo) {
        ResponseVo responseVo = new ResponseVo();
        try {
            HttpResponse<PoiSearchResponseVo> httpResponse  = ticketService.queryStations(requestVo);
            PoiSearchResponseVo poiSearchResponseVo = httpResponse.body();
            responseVo.setSuccess(true);
            responseVo.setMessage(poiSearchResponseVo.getInfo());
            responseVo.setCode(200);
            responseVo.setData(poiSearchResponseVo);
            return responseVo;
        } catch (Exception e) {
            log.error("query stations error:{}",e.getMessage());
            responseVo.setCode(500);
            responseVo.setSuccess(false);
            responseVo.setMessage(e.getMessage());
            return responseVo;
